...
export class ToastManager {
    toastContainer;
    options;
    toastsContainerId = `toasts-${crypto.randomUUID()}`;
    get ToastsCSS() {
        return css `
    ...
    setup() {
        ...
        (this.options.containerElement || document.body).appendChild(toastContainer);
        // set toast container
    ...
    toast(message, type = "default") {
        ...
        if (!this.toastContainer)
            throw new Error("Toast container not set");
...
class Toast {
    ...
    timeoutId = null;
    duration;
    constructor({ message, duration = 3000, type = "default", onClick, }) {
    ...
    animateOut() {
        const animation = this.element.animate([{ opacity: 0, transform: "translateX(250px)" }], {
            duration: 250,
        });
